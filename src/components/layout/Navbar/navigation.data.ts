 ...
 * itself decides, so exactly one top-level item can ever be active.
 */
export type NavActiveMatch = "home" | "urgent-care" | "services" | "occupational-health" | "contact";
...
export function buildNavItems(t: TranslateFn, language: Language): NavItem[] {
  ...
  return [
    ...
    { label: t("navbar.urgentCare"), href: withLocale(SITE.urgentCareHref, language), activeMatch: "urgent-care" },
    {
    ...
    },
    {
      ...
      href: withLocale(SITE.contactHref, language),
      activeMatch: "contact",
    },
  ];
}
