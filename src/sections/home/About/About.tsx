...
export function About() {
  ...
  return (
    <section id="about" className="relative w-full" style={{ background: "linear-gradient(180deg, var(--surface-pale-1) 0%, var(--about-bg-to) 100%)" }}>
      <div className="grid w-full min-h-[520px] grid-cols-[minmax(430px,38%)_minmax(520px,39%)_minmax(300px,23%)] overflow-hidden bg-white mw-1100:min-h-[auto] mw-1100:grid-cols-2 mw-700:w-[calc(100%-32px)] mw-700:grid-cols-1">
        <div
          ...
          style={{ background: "linear-gradient(110deg, #FFFFFF 0%, var(--about-panel-bg) 100%)" }}
        >
          ...
          <h2 className="mt-[22px] max-w-[540px] font-heading text-[clamp(43px,3.5vw,62px)] font-bold leading-[1.03] tracking-[-1.5px] text-text-primary">
            {t("about.headingLine1")}<br />{t("about.headingLine2")}
          </h2>

          <p className="mt-[25px] max-w-[520px] text-[16px] leading-[1.75] text-about-body">
            {t("about.description")}
          </p>

          <div className="mt-9 flex items-start gap-5">
            <div
              className="flex h-[74px] w-[74px] flex-shrink-0 items-center justify-center rounded-full text-white shadow-about-feature-icon"
              style={{ background: "linear-gradient(145deg, var(--about-feature-gradient-from), var(--about-feature-gradient-to))" }}
            >
              <HeartPulseIcon size={30} strokeWidth={1.8} />
            </div>
            <div>
              <h3 className="font-heading text-[20px] font-bold text-about-feature-title">{t("about.feature.title")}</h3>
              <p className="mt-[5px] max-w-[340px] text-[14px] leading-[1.6] text-about-feature-desc">
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, var(--color-brand-dark) 0%, var(--color-brand-dark-end) 55%, var(--about-stats-deep) 100%)" }}
        >
