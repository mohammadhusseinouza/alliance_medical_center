...
const hourRowClass = "flex items-center justify-between border-t border-[#EDF2F4] pt-3";
...
export function ContactHours() {
  ...
  return (
    <section id="hours" className="flex flex-col gap-5">
      <div className="rounded-2xl border border-[#E1EAED] bg-white p-7 shadow-card">
        <div className="flex items-center gap-3">
          ...
          <h2 className="m-0 text-[20px] font-bold text-text-primary">{t("contact.hours.heading")}</h2>
        </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h3 className="m-0 text-[18px] font-bold">{t("contact.hours.emergencyLabel")}</h3>
        </div>
        <a
          href={CONTACT_HOURS.emergencyPhoneHref}
          className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg bg-white px-5 py-2 text-[14.5px] font-bold text-[#0B5F6B] no-underline [transition:background-color_200ms_ease,color_200ms_ease,transform_200ms_ease] hover:-translate-y-px hover:bg-[#1A5560] hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white"
        >
